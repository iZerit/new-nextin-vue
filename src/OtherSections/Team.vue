<template>
  <div class="text-center mt-5 mb-5 services_title">
  <h2>{{ $t('team_title') }}</h2>
  </div>
  <div class="container">
    <div class="main row justify-content-center mb-5 mt-5 ">
      <div class="profile-card mb-5 col-lg-3 col-md-6 col-sm-12">
        <div class="img">
          <img src="/src/assets/team/shohruxTeam.jpg" alt="shohrux">
        </div>
        <div class="caption">
          <h3>{{ $t('shohrux_name') }}</h3>
          <p>{{ $t('shohrux_role') }}</p>

          <div class="social-links">
            <a href="https://www.facebook.com/nextinws"><i class="fab fa-facebook"></i></a>
            <a href="https://www.instagram.com/shokhrukh_malikov/"><i class="fab fa-instagram"></i></a>
            <a href="https://t.me/shokhrukh_malikov"><i class="fab fa-telegram"></i></a>
          </div>
        </div>
      </div>
      <div class="profile-card mb-5 col-lg-3  col-md-6 col-sm-12">
        <div class="img">
          <img src="/src/assets/team/MuhammadAminTeam.jpg" alt="MuhammadAmin">
        </div>
        <div class="caption">
          <h3>{{ $t('muhammadamin_name') }}</h3>
          <p>{{ $t('muhammadamin_role') }}</p>
          <div class="social-links">
            <a href="https://www.facebook.com/profile.php?id=100072760243759"><i class="fab fa-facebook"></i></a>
            <a href="https://www.instagram.com/shukural1yev_o4/"><i class="fab fa-instagram"></i></a>
            <a href="https://t.me/iOutiso"><i class="fab fa-telegram"></i></a>
          </div>
        </div>
      </div>
      <div class="profile-card mb-5 col-lg-3 col-md-6 col-sm-12">
        <div class="img">
          <img src="/src/assets/team/AbduqodirTeam.png" alt="Abduqodir">
        </div>
        <div class="caption">
          <h3>{{ $t('abduqodir_name') }}</h3>
          <p>{{ $t('abduqodir_role') }}</p>
          <div class="social-links">
            <a href="https://www.facebook.com/nextinws"><i class="fab fa-facebook"></i></a>
            <a href="https://www.instagram.com/moon.okean/"><i class="fab fa-instagram"></i></a>
            <a href="https://t.me/Symfony_dev"><i class="fab fa-telegram"></i></a>
          </div>
        </div>
      </div>
      <div class="profile-card mb-5 col-lg-3 col-md-6 col-sm-12">
        <div class="img">
          <img src="/src/assets/team/ibrokhimTeam.png" alt="ibrokhim">
        </div>
        <div class="caption">
          <h3>{{ $t('ibrokhim_name') }}</h3>
          <p>{{ $t('ibrokhim_role') }}</p>
          <div class="social-links">
            <a href="https://www.facebook.com/profile.php?id=100074052992800"><i class="fab fa-facebook"></i></a>
            <a href="https://www.instagram.com/auxetan/"><i class="fab fa-instagram"></i></a>
            <a href="https://t.me/iZerit"><i class="fab fa-telegram"></i></a>
          </div>
        </div>
      </div>
    </div>
  </div>


</template>

<script>
export default {
  name: "Team"
}
</script>

<style scoped>

</style>